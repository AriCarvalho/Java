...
public class Main {
	public static void main(String[] parameters) {
		String[] testParameters = { "-r", "opt1", "-S", "opt2", "arg1", "arg2", "arg3", "arg4", "--test", "-A",
				"opt3", };

		CommandLine commandLine;
		Option option_A = Option.builder("A").required(true).desc("The A option").longOpt("opt3").build();
		Option option_r = Option.builder("r").required(true).desc("The r option").longOpt("opt1").build();
		Option option_S = Option.builder("S").required(true).desc("The S option").longOpt("opt2").build();
		...
		try {
			commandLine = parser.parse(options, testParameters);
